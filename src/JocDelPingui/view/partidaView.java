...
public class partidaView {
    ...
    @FXML private Label dadoResultado;
    @FXML private Button tirarDadoBtn;
    @FXML private Button usarRapidoBtn;
    @FXML private Button usarLentoBtn;
    @FXML private Button usarPezBtn;
    @FXML private Button usarNieveBtn;
    @FXML private Label rapidoCantidad;
    @FXML private Label lentoCantidad;
    @FXML private Label pecesCantidad;
    @FXML private Label nieveCantidad;
    @FXML private VBox eventosLista;
    @FXML private ImageView dadoImagen;
    ...
    private String obtenerClaseCasilla(String tipo) {
        switch(tipo) {
            case "casillaOso": return "casilla-oso";
            case "casillaAgujero": return "casilla-agujero";
            case "casillaTrineo": return "casilla-trineo";
            case "casillaInterrogante": return "casilla-interrogante";
            case "casillaTierraQuebradiza": return "casilla-hielo-roto";
            case "casillaNormal":
                if (descripcionTiene("Meta")) return "casilla-meta";
                return "casilla-nieve";
            default: return "casilla-nieve";
        }
    }
    ...
    private String obtenerIconoCasilla(String tipo) {
        switch(tipo) {
            case "casillaOso": return "🐾";
            case "casillaAgujero": return "💧";
            case "casillaTrineo": return "🛷";
            case "casillaInterrogante": return "🎁";
            case "casillaTierraQuebradiza": return "⚠️";
            case "casillaNormal": return "❄️";
            default: return "❄️";
        }
    }
    ...
    private void crearFichasJugadores() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            switch(i) {
                case 0: circulo.setFill(javafx.scene.paint.Color.web("#E53935")); break;
                case 1: circulo.setFill(javafx.scene.paint.Color.web("#1E88E5")); break;
                case 2: circulo.setFill(javafx.scene.paint.Color.web("#43A047")); break;
                case 3: circulo.setFill(javafx.scene.paint.Color.web("#FDD835")); break;
            }
    ...
    private void handleTirarDado() {
        ...
        // Animación del dado con el resultado
        ...
        animarMovimientoFicha(idxActual, posAnterior, p.getPosicion());
        ...
        jugador nuevoJugador = partida.getJugadores().get(nuevoIdx);
        agregarEvento("🎮 Turno de: Pingüino " + nuevoJugador.getColor());
    }
    ...
    private void animarDado(int resultado) {
        ...
        ScaleTransition st = new ScaleTransition(Duration.millis(300), dadoImagen);
        st.setFromX(1.0);
        st.setFromY(1.0);
        st.setToX(1.5);
        st.setToY(1.5);
        ...
        FadeTransition ft = new FadeTransition(Duration.millis(300), dadoImagen);
        ft.setFromValue(1.0);
        ft.setToValue(0.3);
        ft.setCycleCount(4);
    ...
    private void animarMovimientoFicha(int jugadorIdx, int desde, int hasta) {
        if (jugadorIdx >= fichasJugadores.size()) return;
        
        StackPane ficha = fichasJugadores.get(jugadorIdx);
        
        // Calcular dirección del movimiento
        int filaDesde = desde / 10;
        int colDesde = desde % 10;
        int filaHasta = hasta / 10;
        int colHasta = hasta % 10;
        
        // Obtener tamaño actual de las casillas (dinámico)
        double anchoCasilla = tableroGrid.getWidth() / 10;
        double altoCasilla = tableroGrid.getHeight() / 5;
        
        double deltaX = (colHasta - colDesde) * anchoCasilla;
        double deltaY = (filaHasta - filaDesde) * altoCasilla;
        
        TranslateTransition tt = new TranslateTransition(Duration.millis(500), ficha);
        tt.setByX(deltaX);
        tt.setByY(deltaY);
        tt.setCycleCount(1);
    ...
    private void handleUsarNieve() {
        ...
        if (p.getInventario().getBolasNieve() > 0) {
            ...
            animarMovimientoFicha(objetivo, objetivoJugador.getPosicion() + 3, 
                                  objetivoJugador.getPosicion());
    ...
    private void agregarEvento(String mensaje) {
        ...
        String icono = "📢";
        if (mensaje.contains("Turno")) icono = "🎮";
        else if (mensaje.contains("nieve") || mensaje.contains("❄️")) icono = "❄️";
        else if (mensaje.contains("oso") || mensaje.contains("🐻")) icono = "🐻";
        else if (mensaje.contains("pez") || mensaje.contains("🐟")) icono = "🐟";
        else if (mensaje.contains("rápido") || mensaje.contains("⚡")) icono = "⚡";
        else if (mensaje.contains("lento") || mensaje.contains("🐢")) icono = "🐢";
    ...
    private void configurarResponsive() {
        // Escalar las casillas según el tamaño del GridPane
        tableroGrid.widthProperty().addListener((obs, oldVal, newVal) -> {
            double anchoCasilla = newVal.doubleValue() / 10;
            for (StackPane casilla : casillasGraficas) {
                casilla.setPrefWidth(anchoCasilla - 5);
                casilla.setMaxWidth(anchoCasilla - 5);
                casilla.setMinWidth(anchoCasilla - 5);
            }
        });
        
        tableroGrid.heightProperty().addListener((obs, oldVal, newVal) -> {
            double altoCasilla = newVal.doubleValue() / 5;
            for (StackPane casilla : casillasGraficas) {
                casilla.setPrefHeight(altoCasilla - 5);
                casilla.setMaxHeight(altoCasilla - 5);
                casilla.setMinHeight(altoCasilla - 5);
            }
        });
    }
}